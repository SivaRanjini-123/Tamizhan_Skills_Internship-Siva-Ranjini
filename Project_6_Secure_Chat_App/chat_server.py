...
def exchange_keys(sock, my_public_key_pem):
    ...
    print(f"[DEBUG SERVER] Sent my public key (length: {key_len} bytes).")

    # 2. Receive other participant's public key with length prefix
    print("[DEBUG SERVER] Waiting to receive other participant's public key length...")
    ...
    print(f"[DEBUG SERVER] Expected other public key length: {other_key_len} bytes.")
    ...
    print(f"[DEBUG SERVER] Received other public key (actual length: {len(other_public_key_pem)} bytes).")
    # print(f"[DEBUG SERVER] Received PEM:\n{other_public_key_pem.decode()}") # Uncomment for detailed debug
    ...
    try:
        ...
        print(f"[-] [ERROR SERVER] Failed to import other public key: {e}")
        print(f"[-] [ERROR SERVER] Received data that caused error:\n{other_public_key_pem}")
...
def receive_messages(sock, private_key_obj):
    ...
    while True:
        try:
            encrypted_msg_b64 = sock.recv(4096) # Receive up to 4KB of data
...
def start_server():
    ...
    print(f"[DEBUG SERVER] My private key object type: {type(my_private_key_obj)}")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            print(f"Error binding socket: {e}. Make sure no other process is using port {PORT}.")
        ...
        with conn:
            ...
            # Key Exchange
            other_public_key_obj = exchange_keys(conn, my_public_key_pem)
            print("Key exchange complete.")

            # Start receiving messages in a separate thread
            receive_thread = threading.Thread(target=receive_messages, args=(conn, my_private_key_obj)) # Pass the key object
